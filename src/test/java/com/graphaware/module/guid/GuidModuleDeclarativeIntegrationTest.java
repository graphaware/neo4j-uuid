...
import com.graphaware.module.guid.api.GuidApi;
import com.graphaware.module.guid.generator.EaioUUIDGenerator;
import com.graphaware.module.guid.generator.GuidGenerator;
import com.graphaware.module.guid.generator.JavaUtilUUIDGenerator;
import com.graphaware.module.guid.generator.SequenceIdGenerator;
import com.graphaware.runtime.policy.all.IncludeAllBusinessNodes;
import com.graphaware.runtime.policy.all.IncludeAllBusinessRelationships;


public class GuidModuleDeclarativeIntegrationTest {
    ...
    @Test
    public void testGuidAssigned() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();

        getRuntime(database).waitUntilStarted();
        GuidApi api = new GuidApi(database);
        //When
        ...
        try (Transaction tx = database.beginTx()) {
            for (Node node : asIterable(database.findNodes(personLabel))) {
                ...
                assertEquals(Long.valueOf(node.getId()), api.getNodeIdByGuid((String) node.getProperty(GUID)));
            }
            tx.success();
        }
    }
    ...
    @Test
    public void testGuidAssignedWithoutHyphens() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid-strip-hyphens.conf").getPath())
                .newGraphDatabase();
        ...
        try (Transaction tx = database.beginTx()) {
            for (Node node : asIterable(database.findNodes(personLabel))) {
                assertTrue(node.hasProperty(GUID));
                assertFalse(node.getProperty(GUID).toString().contains("-"));
                assertEquals(Long.valueOf(node.getId()), api.getNodeIdByGuid((String) node.getProperty(GUID)));
            }
            tx.success();
        }
    ...
    public void testDefaultGenerator() {
        ...
        GuidModule guidModule = getRuntime(database).getModule(GuidModule.class);
        GuidGenerator GuidGenerator = guidModule.getGuidGenerator();
        Assert.assertEquals(EaioUUIDGenerator.class, GuidGenerator.getClass());

    }
    
    @Test
    public void testGuidGeneratorJavaUtilGUID() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid-generator-java-util.conf").getPath())
                .newGraphDatabase();

        // Verify the generator instantiated is as expected
        GuidModule guidModule = getRuntime(database).getModule(GuidModule.class);
        GuidGenerator GuidGenerator = guidModule.getGuidGenerator();
        Assert.assertEquals(JavaUtilUUIDGenerator.class, GuidGenerator.getClass());
    ...
    @Test
    public void testGuidGeneratorSequenceIdGenerator() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid-generator-sequenceid.conf").getPath())
                .newGraphDatabase();

        // Verify the generator instantiated is as expected
        GuidModule guidModule = getRuntime(database).getModule(GuidModule.class);
        GuidGenerator GuidGenerator = guidModule.getGuidGenerator();
        Assert.assertEquals(SequenceIdGenerator.class, GuidGenerator.getClass());
    ...
    public void testGuidGeneratorInvalidGenerator()  {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid-generator-invalid.conf").getPath())
                .newGraphDatabase();

        // This should cause the expected exception due to an invalid generator being configured
        getRuntime(database).getModule(GuidModule.class);
        
    }
    
    @Test
    public void testGuidNotAssigned() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();
        ...
        try (Transaction tx = database.beginTx()) {
            for (Node node : asIterable(database.findNodes(ignoredLabel))) {
                assertFalse(node.hasProperty(GUID));
            }
            tx.success();
        }
    }
    ...
    @Test(expected = NotFoundException.class)
    public void testGetNodeThrowsExceptionForInvalidGuid() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();
        ...
        try (Transaction tx = database.beginTx()) {
            assertNull(api.getNodeIdByGuid("xyz"));
            tx.success();
        }
    }

    @Test
    public void testGuidAssignedToRelationship() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();
        ...
        //Then
        //Retrieve the nodes and relationships and check that it has a guid property
        try (Transaction tx = database.beginTx()) {
            for (Node node : asIterable(database.findNodes(personLabel))) {
                assertTrue(node.hasProperty(GUID));
                assertEquals(Long.valueOf(node.getId()), api.getNodeIdByGuid((String) node.getProperty(GUID)));
            ...
            for (Relationship rel : database.getAllRelationships()) {
                assertTrue(rel.hasProperty(GUID));
                assertEquals(Long.valueOf(rel.getId()), api.getRelationshipIdByGuid((String) rel.getProperty(GUID)));
            }
            tx.success();
        }
    }
    ...
    @Test
    public void testGuidNotAssignedToRelationship() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();
    ...
    @Test(expected = NotFoundException.class)
    public void testGetRelationshipThrowsExceptionForInvalidGuid() throws InterruptedException {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid.conf").getPath())
                .newGraphDatabase();
        ...
        GuidApi api = new GuidApi(database);
        try (Transaction tx = database.beginTx()) {
            assertNull(api.getRelationshipIdByGuid("xyz"));
            tx.success();
        }
    }
    ...
    @Test
    public void longCypherCreateShouldResultInAllNodesAndRelsWithGuid() {
        database = new TestGraphDatabaseFactory().newImpermanentDatabaseBuilder()
                .loadPropertiesFromFile(this.getClass().getClassLoader().getResource("neo4j-guid-all.conf").getPath())
                .newGraphDatabase();
